...
# bzip2 -dc kowiki-latest-pages-articles.xml.bz2 | WikiExtractor.py -cb 250K -o extracted
#
...
class OutputSplitter:
    def __init__(self, compress, max_file_size, path_name, file_name):
        self.__dir_index = 0
        self.__file_index = -1
    ...
    def __get_dir_name(self):
        ...
        return self.__path_name # os.path.join(self.__path_name, '%c%c' % (ord('A') + char2, ord('A') + char1))
...
def process_data(input_file, wiki_extractor, output_splitter, prefix):
    page = []
    for line in fileinput.input([input_file]):
        ...
        if line == '<page>':
            ...
            process_page(page, wiki_extractor, output_splitter, prefix)
        else:
            page.append(line)

#------------------------------------------------------------------------------

def process_page(page, wiki_extractor, output_splitter, prefix):
    wiki_document = extract_document(page, prefix)
...
def extract_document(page, prefix):
...
def main():
    ...
    try:
        long_opts = ['help', 'usage', 'compress', 'bytes=', 'output=', 'lang=', 'input=']
        opts, args = getopt.gnu_getopt(sys.argv[1:], 'cb:o:l:i:', long_opts)
    ...
    prefix = 'http://'+language+'.wikipedia.org/wiki/'

    wiki_extractor = WikiExtractor()
    output_splitter = OutputSplitter(compress, file_size, output_dir, language + 'wiki.xml')
    process_data(input, wiki_extractor, output_splitter, prefix)
