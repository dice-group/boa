...
public class SupportFeature implements Feature {
	...
	public void score(FeatureExtractionPair pair) {
		...
		double maxLearnedFrom = (double) (Math.log((pair.getPattern().getMaxLearnedFrom() + 1)) / Math.log(2));
		double countLearnedFrom = (double) (Math.log((pair.getPattern().getLearnedFromPairs() + 1)) / Math.log(2));
		
		pair.getPattern().getFeatures().put(de.uni_leipzig.simba.boa.backend.entity.pattern.feature.enums.Feature.SUPPORT_NUMBER_OF_MAX_PAIRS_LEARNED_FROM, maxLearnedFrom >= 0 ? maxLearnedFrom : 0);
		pair.getPattern().getFeatures().put(de.uni_leipzig.simba.boa.backend.entity.pattern.feature.enums.Feature.SUPPORT_NUMBER_OF_PAIRS_LEARNED_FROM, countLearnedFrom  >= 0 ? countLearnedFrom : 0);
