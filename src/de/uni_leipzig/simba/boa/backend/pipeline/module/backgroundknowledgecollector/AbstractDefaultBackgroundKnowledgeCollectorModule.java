 ...
 */
public abstract class AbstractDefaultBackgroundKnowledgeCollectorModule extends
		AbstractBackgroundKnowledgeCollectorModule {

	private final NLPediaLogger logger = new NLPediaLogger(
			AbstractDefaultBackgroundKnowledgeCollectorModule.class);

	protected final String SPARQL_ENDPOINT_URI = NLPediaSettings
			.getSetting("dbpediaSparqlEndpoint");
	protected final String DBPEDIA_DEFAULT_GRAPH = NLPediaSettings
			.getSetting("sparqlDefaultGraph");
	private final int SPARQL_QUERY_LIMIT = NLPediaSettings
			.getIntegerSetting("sparqlQueryLimit");
	protected final String BACKGROUND_KNOWLEDGE_OUTPUT_PATH = NLPediaSettings.BOA_DATA_DIRECTORY
			+ de.uni_leipzig.simba.boa.backend.Constants.BACKGROUND_KNOWLEDGE_PATH;
	...
	public void updateModuleInterchangeObject() {
		...
		this.moduleInterchangeObject.getBackgroundKnowledge().addAll(
				this.backgroundKnowledge);
		for (BackgroundKnowledge bk : this.backgroundKnowledge) {

			this.moduleInterchangeObject.getProperties().put(
					bk.getProperty().getUri().hashCode(), bk.getProperty());
		}
		...
		while (true) {
			...
			if (!resultSetList.isEmpty()) {
				...
				if (query.contains("?o rdfs:label ?ol")) {
					...
					handleDatatypePropertyQuery(property, fileName,
							resultSetList);
				}
	 ...
	 */
	private void handleDatatypePropertyQuery(Property property,
			String fileName, List<QuerySolution> resultSets) {

		BufferedFileWriter writer = FileUtil.openWriter(fileName,
				Constants.UTF_8_ENCODING, WRITER_WRITE_MODE.APPEND);
		...
		for (QuerySolution solution : resultSets) {
			...
			if (solution.get("oLabel") != null) {
				...
				if (objectUri.contains("^^"))
					objectType = objectUri.substring(objectUri
							.lastIndexOf("^^") + 2);
				if (objectUri.contains("^^"))
			...
			datatypeBackgroundKnowledge.setSubjectPrefixAndLocalname(solution
					.get("s").toString());
			datatypeBackgroundKnowledge.setSubjectLabel(subjectLabel
					.replaceAll("\\(.+?\\)", "").trim());

			datatypeBackgroundKnowledge.setObjectPrefixAndLocalname(objectUri);
			datatypeBackgroundKnowledge.setObjectLabel(objectLabel.replaceAll(
					"\\(.+?\\)", "").trim());
			datatypeBackgroundKnowledge.setObjectDatatype(objectType);

			datatypeBackgroundKnowledge.setProperty(property);

			BackgroundKnowledge backgroundKnowledge = SurfaceFormGenerator
					.getInstance().createSurfaceFormsForBackgroundKnowledge(
							datatypeBackgroundKnowledge);
	...
	protected void handleObjectPropertyQuery(Property property,
		...
		for (QuerySolution solution : resultSets) {
			...
			if (solution.get("s") != null && solution.get("callret-2") != null
				...
				BackgroundKnowledge backgroundKnowledge = SurfaceFormGenerator
						.getInstance()
						.createSurfaceFormsForBackgroundKnowledge(
								objectBackgroundKnowledge);
	...
	protected Property queryPropertyData(String propertyUri) {
		...
		if (this.properties.containsKey(property.hashCode())) {
			...
			QueryEngineHTTP qexecProperty = new QueryEngineHTTP(
					SPARQL_ENDPOINT_URI, propertyQuery);
			...
			if (results.hasNext()) {
				...
				String domain = qs.get("domain") == null ? "NA" : qs.get(
						"domain").toString();
				String range = qs.get("range") == null ? "NA" : qs.get("range")
						.toString();

				p = new Property(propertyUri, domain, range);
				p.setSynsets(WordnetQuery.getSynsetsForAllSynsetTypes(p
						.getLabel()));
