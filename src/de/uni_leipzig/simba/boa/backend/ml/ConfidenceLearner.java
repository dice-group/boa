...
public class ConfidenceLearner {
     ...
     * the training data is used to train the best possible network
     * @param inputFile Tab-separated input file; last column is 1 = good or 0 false
     * @param outputFile Output file for evaluation
     * @param networkFile 
     * @param n n for n-fold
     * @param repetitions number of repetitions of an experiments
     */
     ...
     * to network file. Writes eval results to outputFile. Repeats each experiments
     * once.
     * 
     * @param inputFile Tab-separated input file; last column is 1 = good or 0 false
     * @param outputFile Output file for evaluation
            ...
            for (int hidden = inputSize; hidden < maxHiddenToInputRatio * inputSize + 1; hidden++) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    for (int rep = 0; rep < repetitions; rep++) {
                        accuracy = trainAndEvaluate(trainingData, testData, inputSize, hidden,
                                1, error);
    ...
    /** Merge all data included in alldata with the exception of nth one to training data
     * The nth data set is used for testing
     * @param allData All data available
     * @param exception Data set to use for testing
     * @param n Number of slices to creates (n-fold cross evaluation)
     ...
     * @param hiddenSize1 Size of first hidden layer
     * @param hiddenSize2 Size of second hidden layer
     * @param outputSize Size of output layer
     * @return Network
     */
    public BasicNetwork generateHiddenNetwork(int inputSize, int hiddenSize1,
            int hiddenSize2, int outputSize) {
        BasicNetwork network = new BasicNetwork();
        network.addLayer(new BasicLayer(null, true, inputSize));
     ...
     * @return 
     */
    public BasicNetwork trainToFile(String inputDataFile, int inputSize, int hiddenSize, int outputSize,
            String outputFile, double error) {
     ...
     * @param hiddenSize Size of hidden layer
     * @param outputSize Size of output layer
     * @param error Error to train to
     * @return Percentage of correctly classified patterns from test data
     */
    public double trainAndEvaluate(MLDataSet trainingData, MLDataSet testData,
            int inputSize, int hiddenSize, int outputSize, double error) {
     ...
     * @param maxEpochs Maximal number of epochs
     */
...
//                    + " Target Error: " + Format.formatPercent(error));
     ...
     * 
     * @param netFile File
     * @param data Machine Learning Data
     * @return Score
     */
     ...
     * 
     * @param netFile Input file
     * @return Network
     */
    public static BasicNetwork getNetwork(String netFile) {
        try {
            //return (BasicNetwork)EncogDirectoryPersistence.loadObject(new File(netFile));
    ...
    /** Computes the score of an entry by using the network in the confidence learner
     * 
     * @param entry Entry
     * @return Confidence
     */
    ...
    public static void main(String args[]) {
        ...
        String resultsFile = "D:/tmp/results.log";
        dr.runEval("D:/Work/Java/ConfidenceClassifier/test/dummy.txt", resultsFile,
                "D:/tmp/conf.log", 3, 3);
        BasicNetwork net = ConfidenceLearner.getNetwork("D:/tmp/conf.log");
        String input = "1.0\t1.0";
        MLData data = ConfidenceLearner.getSingleEntry(input);
        double conf = ConfidenceLearner.getConfidence(net, input);
