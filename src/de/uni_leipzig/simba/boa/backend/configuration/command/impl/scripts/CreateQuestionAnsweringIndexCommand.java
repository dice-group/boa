...
public class CreateQuestionAnsweringIndexCommand { //implements Command {
	...
	public static void execute() {
		...
		try {
			...
			IndexWriter writer = new IndexWriter(indexDirectory, analyzer, false, IndexWriter.MaxFieldLength.LIMITED);
			...
			for ( PatternMapping mapping : patternMappings ) {
				...
				for (Pattern pattern : mapping.getPatterns() ) {
					...
					doc.add(new Field("nlr", pattern.getNaturalLanguageRepresentationWithoutVariables(), Field.Store.YES, Field.Index.ANALYZED));
	...
	public static void main(String[] args) throws ParseException, CorruptIndexException, IOException {
		...
		String searchPhrase = "born in";
		double confidenceThreshold = 0.6D;
		...
		for (int i = 0; i < hits.length && i < 5; i++) {
			...
			uris.add(indexSearcher.doc(hits[i].doc).get("uri"));
		}
	}
}
